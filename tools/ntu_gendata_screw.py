...
from utils.ntu_read_skeleton import read_xyz, plucker
...
def gendata(data_path,
    ...
    with open('{}/{}_screwlabel.pkl'.format(out_path, part), 'wb') as f:
        pickle.dump((sample_name, list(sample_label)), f)
    # np.save('{}/{}_label.npy'.format(out_path, part), sample_label)

    fp = open_memmap(
        '{}/{}_screwdata.npy'.format(out_path, part),
        dtype='float32',
        mode='w+',
    ...
    for i, s in enumerate(sample_name):
        ...
        #data = extend_rotate(os.path.join(data_path, s), max_body=max_body, num_joint=num_joint)
        data = plucker(os.path.join(data_path, s), max_body=max_body, num_joint=num_joint)
